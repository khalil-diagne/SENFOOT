...
$pendingSellers = $pdo->query('SELECT COUNT(*) FROM visiteur WHERE seller_verified = 2')->fetchColumn();
$pendingSellerArticles = $pdo->query("SELECT COUNT(*) FROM articles WHERE approval_status = 'pending' AND author_username IS NOT NULL")->fetchColumn();
...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .nav-badge{margin-left:auto;min-width:22px;height:22px;padding:0 7px;border-radius:999px;background:var(--neon-red);color:#fff;font-family:'Orbitron',sans-serif;font-size:10px;font-weight:700;display:inline-flex;align-items:center;justify-content:center;box-shadow:0 0 12px rgba(255,68,102,0.45);}
        ...
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="pending_sellers.php" class="nav-item">
                    <span class="nav-icon">ID</span> Candidatures vendeurs
                    <?php if ((int) $pendingSellers > 0): ?>
                        <span class="nav-badge"><?= (int) $pendingSellers ?></span>
                    <?php endif; ?>
                </a>
                <a href="pending_articles.php" class="nav-item">
                    <span class="nav-icon">OK</span> Articles vendeurs
                    <?php if ((int) $pendingSellerArticles > 0): ?>
                        <span class="nav-badge"><?= (int) $pendingSellerArticles ?></span>
                    <?php endif; ?>
                </a>
                ...
                <div class="dashboard-grid">
                    ...
                    <div class="panel">
                        ...
                        <div class="actions-list">
                            ...
                            <a href="pending_sellers.php" class="action-link">
                                <span class="action-link-icon">ID</span>
                                Voir les candidatures vendeurs
                                <?php if ((int) $pendingSellers > 0): ?>
                                    <span class="nav-badge"><?= (int) $pendingSellers ?></span>
                                <?php endif; ?>
                                <span class="action-link-arrow">→</span>
                            </a>
                            <a href="pending_articles.php" class="action-link">
                                <span class="action-link-icon">OK</span>
                                Valider les articles vendeurs
                                <?php if ((int) $pendingSellerArticles > 0): ?>
                                    <span class="nav-badge"><?= (int) $pendingSellerArticles ?></span>
                                <?php endif; ?>
                                <span class="action-link-arrow">→</span>
